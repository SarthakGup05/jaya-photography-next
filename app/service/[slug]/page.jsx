...
const ServicePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchServiceData = async () => {
      try {
        ...
        const message = error.response?.data?.error || "Failed to load service";
        setError(message);

        if (error.response?.status === 404) {
          toast.error("Service not found");
          setTimeout(() => router.push("/services"), 2500);
        } else {
  ...
  const handleBookingSubmit = async (formData) => {
    try {
      ...
      toast.success(`Booking request for "${serviceData.title}" sent successfully!`);
    } catch (error) {
      const message =
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#f3e6fa] to-[#F0E7E5]">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-800">Loading Service</h3>
  ...
  const seoTitle =
    ...
    `${serviceData.title} - ${serviceData.subtitle || ""} | Jaya Photography`;
  const seoDescription =
    serviceData.metaDescription ||
    serviceData.description?.slice(0, 160) ||
    "Explore professional photography services at Jaya Photography.";
    ...
    <>
      ...
      <Head>
        ...
        {serviceData.mainImage && (
          <meta property="og:image" content={serviceData.mainImage} />
        )}
      </Head>
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                {serviceData.duration && (
                  <div className="flex items-center gap-3 mt-4 p-3 bg-purple-50/50 rounded-xl border border-purple-100">
                    ...
                    <span className="text-sm text-gray-700 font-medium">
                      Session Duration: {serviceData.duration}
                    </span>
                  </div>
                )}
              </div>
              ...
              <Suspense fallback={<div className="text-center text-gray-500">Loading form...</div>}>
                <Modal
                  ...
                  title={`📸 Book "${serviceData.title}"`}
                  description={`Fill your details below to book a ${serviceData.title} session.`}
                  ...
                  {({ close }) => (
                    <ContactForm
                      initialData={{
                        ...
                        message: `Hello! I'm interested in booking a "${serviceData.title}" session.`,
                      }}
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-6">
                <div className="relative overflow-hidden rounded-2xl shadow-lg bg-white/20 border border-white/30">
                  <img
                    src={serviceData.mainImage || serviceData.coverImage || "/api/placeholder/400/300"}
